...
- **Deploy**: projeto único na Vercel; rewrite de `/` para a landing e `/painel/*` para o build React (`painel-app/dist/`, com fallback SPA para `index.html`). Build no deploy: `npm run build -w precifica-painel` com `outputDirectory: "."` (a raiz é servida — sem isso o deploy falha com "No Output Directory named public"). O fonte React vive em `painel-app/` e o build sai em `painel-app/dist/` — NUNCA deixar um `painel/index.html` estático no output (o arquivo encobre o rewrite e quebra `/painel/` com 404). Env `VITE_*` cadastradas no dashboard da Vercel. Sem Render e sem domínio próprio por enquanto.
